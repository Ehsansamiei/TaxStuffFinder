...
public class ExcelService
{
    ...
    public Dictionary<string, List<StuffModel>> SearchAndGroup(string searchTerm, string groupVatRate, string groupTypeStuffId)
    {
        var filteredData = string.IsNullOrWhiteSpace(searchTerm)
            ...
            : _mergedExcelData
                .Where(item =>
                    item.StuffID.Contains(searchTerm, StringComparison.OrdinalIgnoreCase) ||
                    item.Descript.Contains(searchTerm, StringComparison.OrdinalIgnoreCase) ||
                    item.VatRate.Contains(searchTerm, StringComparison.OrdinalIgnoreCase) ||
                    item.EditDateFa.Contains(searchTerm, StringComparison.OrdinalIgnoreCase) ||
                    item.typeStuffid.Contains(searchTerm, StringComparison.OrdinalIgnoreCase))
                .ToList();
        ...
        {
            // Group by both TypeStuffId and VatRate
            return filteredData
                .GroupBy(item => $"{item.typeStuffid}|{item.VatRate}")
                .ToDictionary(
                    g => g.Key,
                    g => g.ToList()
                );
